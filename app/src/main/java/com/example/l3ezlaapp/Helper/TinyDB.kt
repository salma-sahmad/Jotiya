...
class TinyDB(private val appContext: Context) {
    ...
    fun putImageWithFullPath(fullPath: String?, theBitmap: Bitmap?): Boolean {
        if (fullPath == null || theBitmap == null) return false
    ...
    fun getListInt(key: String): ArrayList<Int> {
        ...
        val arrayToList = ArrayList(myList.asList())
    ...
    fun getListString(key: String): ArrayList<String> {
        return ArrayList(TextUtils.split(preferences.getString(key, ""), "‚‗‚").asList())
    }
    ...
    fun getListBoolean(key: String): ArrayList<Boolean> {
        ...
        for (item in myList) newList.add(item.toBoolean())
        return newList
    }
    ...
    fun getListObject(key: String): ArrayList<ItemModel> {
        ...
        for (jObjString in objStrings) {
            val player = gson.fromJson(jObjString, ItemModel::class.java)
